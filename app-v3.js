/* ===================== 得闲茶吧 · 经营工作台 ===================== */
/* 单页应用：本地自动保存 + JSONBin 云端同步（可选） */

// 安全兜底：防止旧版缓存代码调用不存在的函数
if(typeof window.defn==='undefined') window.defn=function(){};
...
function init(){
  ...
  // 不再注册 Service Worker（避免缓存干扰）
  // if('serviceWorker' in navigator){ navigator.serviceWorker.register('sw-reset.js').catch(()=>{}); }
}
